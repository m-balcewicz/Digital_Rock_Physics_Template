"""
Effective Medium and Mixing Laws
==================================

Fluid and solid mixing calculations for rock physics modeling.

This module provides functions for calculating effective properties of
multi-phase mixtures using various mixing laws and averaging schemes.
...
Functions
---------
- density_solid_mix: Calculate density of solid mixture
- density_fluid_mix: Calculate density of fluid mixture
- Brie_law: Brie's equation for patchy fluid mixtures
- get_normalized_f_solid: Normalize solid fractions based on porosity
- elastic_bounds: Calculate Voigt, Reuss, or Hashin-Shtrikman bounds
"""
...
def density_solid_mix(f_solid, rho_solid):
    """
    Calculate the effective density of a solid mixture.
    
    f_volume = f_porosity + f_solid
    
    Args:
        f_solid (array-like): Fractions of the solid components. Must sum to 1.
        rho_solid (array-like): Densities of the solid components (kg/m³).
        
    Returns:
        float: Mixed solid density (kg/m³).
        
    Raises:
        ValueError: If input arrays have different lengths.
        ValueError: If volume fractions don't sum to 1.
    """
...
def density_fluid_mix(s_fluid, rho_fluid):
    """
    Calculate the effective density of a fluid mixture.
    
    Args:
        s_fluid (array-like): Saturation of the fluid phases. Must sum to 1.
        rho_fluid (array-like): Densities of the fluid components (kg/m³).
        
    Returns:
        float: Mixed fluid density (kg/m³).
        
    Raises:
        ValueError: If input arrays have different lengths.
        ValueError: If saturations don't sum to 1.
    """
...
def Brie_law(s_water, s_oil, s_gas, k_water, k_oil, k_gas, e=3):
    ...
    Args:
        s_water (float): Water saturation (0-1).
        s_oil (float): Oil saturation (0-1).
        s_gas (float): Gas saturation (0-1).
        k_water (float): Bulk modulus of water (Pa).
        k_oil (float): Bulk modulus of oil (Pa).
        k_gas (float): Bulk modulus of gas (Pa).
        e (float, optional): Empirical constant. Default is 3 (original experiments).
        
    Returns:
        float: Effective bulk modulus using Brie's law (Pa).
...
def get_normalized_f_solid(porosity, f_solid_components, components=None):
    ...
    The function utilizes NumPy and pandas for efficient array operations and data manipulation.
    ...
    - components (list of str, optional): List of component names corresponding to the solid phases.

    Returns:
    - result_df (pandas.DataFrame): A DataFrame containing the normalized solid fractions,
      corresponding component names, and the "Porosity" column.
    ...
    components = ['Quartz', 'Feldspar', 'Dolomite']

    result_df = get_normalized_f_solid(porosity, f_solid_components, components=components)
    ```
    """
    ...
    if f_solid_components.ndim == 1:
        if components is None or len(components) != f_solid_components.shape[0]:
            raise ValueError('Invalid number of components. Provide a list of component names with the correct length.')
...
def elastic_bounds(fractions, k_values, u_values, bound_type='voigt-reuss'):
    ...
    - k_values (numpy.ndarray): Bulk moduli of the components (Pa).
    - u_values (numpy.ndarray): Shear moduli of the components (Pa) (not used for 'fluid' bound_type).
    - bound_type (str): Type of bound calculation:
        - 'voigt-reuss': Simplest bounds (default)
