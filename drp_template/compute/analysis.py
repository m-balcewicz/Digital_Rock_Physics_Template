...
def porosity(data, paramsfile='parameters.json'):
    # # Initialization
...
def phase_fractions(data, labels=None, paramsfile='parameters.json', verbose=False, **kwargs):
    """
    ...
    table = compute.phase_fractions(data, verbose=True)
    
    # With labels for better readability
    labels = {"0": "Pore", "1": "Quartz", "2": "Feldspar"}
    table = compute.phase_fractions(data, labels=labels, verbose=True)
    
    # Save to custom parameters file
    table = compute.phase_fractions(data, labels=labels, 
                                    paramsfile='my_model.json', verbose=True)
    ```
